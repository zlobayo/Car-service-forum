...
<?php    
    if (isset($_POST['create_profile'])) {
        try {
            if (empty($_POST['name'])) {
                throw new Exception("<br>Your name cannot be empty.");
            } else {
                $name_input = htmlentities($_POST['name']);
            }
            if (empty($_POST['username'])) {
                throw new Exception("<br>Username cannot be empty");
            } else {
                $username_input = htmlentities($_POST['username']);
            }
            if (empty($_POST['password'])) {
                throw new Exception("<br>Password cannot be empty");
            } else {
                $password_input = htmlentities($_POST['password']);
            }
            if (empty($_POST['email'])) {
                throw new Exception("<br>E-mail cannot be empty");
            } else {
                $email_input = htmlentities($_POST['email']);
            }           
        } catch (Exception $e){
